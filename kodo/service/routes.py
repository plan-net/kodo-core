...
class NodeConnector(kodo.service.controller.Controller):
    ...
    async def get_map(
        ...
        try:
            default = kodo.service.controller.default_response(
                state).model_dump()
            ...
            logger.error(
                f"Failed to retrieve provider and connection map: {e}")
            raise HTTPException(
                status_code=HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to retrieve provider and connection map: {
                    str(e)}"
            )
    ...
    async def connect(
        ...
        try:
            ...
            logger.info(f"Connected {data.url} with {helper.stat(data.nodes)}; feed={
                        data.feed} to organization {data.organization}")
                ...
                for peer in state.providers.values():
                    if peer.feed:
                        if peer.url != data.url:
                            try:
                                logger.debug(f"Broadcasting connection to {
                                             peer.url}/connect with {helper.stat(data.nodes)}")
                                kodo.service.signal.emit(
                                    request.app, "update", peer.url, state, feed)
                            except Exception as e:
                                logger.warning(f"Failed to notify peer {
                                               peer.url}: {e}")
        ...
        try:
            ...
            if data.provider not in state.providers:
                raise HTTPException(
                    ...
                    detail=f"Provider {
                        data.provider} not found in the registry."
                )
                    ...
                    for peer in state.providers.values():
                        if peer.feed and peer.url != data.provider:
                            ...
                            try:
                                ...
                                logger.error(f"Error notifying peer {
                                             peer.url}: {e}")

            else:
                raise HTTPException(status_code=HTTP_404_NOT_FOUND, detail=f"Provider {
            ...
            for url in state.connection.keys():
                try:
                    resp = httpx.post(
                        ...
                        timeout=None)
                    logger.debug(f"Disconnect response from {
                    ...
                    if resp.status_code not in [HTTP_200_OK, HTTP_201_CREATED, HTTP_204_NO_CONTENT]:
                        logger.error(f"Failed to disconnect from {
                                     url}: {resp.json()}")
    ...
    async def reconnect(
        ...
        try:
            ...
            raise HTTPException(
                status_code=HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Unexpected error while reconnecting to {
                    data.url}: {e}"
            )
